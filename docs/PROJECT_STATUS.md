...
### 次のステップ
0. 🔜 **次に着手予定: レスポンシブデザイン化（2026-06-12 ユーザー要望・最優先）**。スマホ／タブレット等の画面幅で全ページが崩れず使えるようにする。対象は全画面（ランディング・認証・ダッシュボード・サイドバー・`/onboarding`・`/learn/typing`・`/learn/toeic`・`/learn/toeic/part5`・`/history` 等）。Tailwind のブレークポイント（`sm`/`md`/`lg`）で対応。**このステップは未実装**。
...
- **レスポンシブデザイン化の要望（2026-06-12）**: 現状の UI を**スマホ／タブレットなど各画面幅に対応（レスポンシブ）**させてほしいとの要望。次の実装タスクとして最優先で着手する（このコミット時点では記録のみ・未実装）。理由: PC 前提のレイアウトのままだと、Vercel デプロイ後にスマホで確認する際に崩れるため。方針: Tailwind のブレークポイントで全ページを段階的に対応（サイドバーのモバイル折りたたみ、カード/グリッドの段組み変更、文字・余白の調整など）。
