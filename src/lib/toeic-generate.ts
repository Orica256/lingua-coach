...
- **可読性のため passage は段落を空行（\\n\\n）で区切る**。例: 宛名/件名 → 本文（1〜2段落）→ 結び・署名 をそれぞれ別段落にし、各段落の間に空行を入れる。1つの塊にしない。
...
- **可読性のため passage は段落を空行（\\n\\n）で区切る**。導入 → 本論（複数段落）→ 結びのように 2〜4 段落に分け、各段落の間に必ず空行を入れる。長い1ブロックにしない。Eメール形式なら宛名・本文・署名も段落で分ける。
